...
test('trust gate accepts the aligned release and database pins', () => {
  ...
  try {
    assert.match(check(fixtureRoot), /release v0\.8\.12/);
  } finally {
...
test('trust gate rejects a stale AuthOS Compose pin', () => {
  ...
  try {
    ...
    fs.writeFileSync(
      composePath,
      fs.readFileSync(composePath, 'utf8').replace('sqlite-v0.8.12', 'sqlite-v0.8.11'),
    );
    assert.throws(
      () => check(fixtureRoot),
      (error) => error?.status === 1 && /does not pin editoredit\/sso:sqlite-v0\.8\.12/.test(error.stderr),
    );
  } finally {
    fs.rmSync(fixtureRoot, { recursive: true, force: true });
  }
