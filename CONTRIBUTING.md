...
AuthOS uses path-specific licenses. The API is AGPL-3.0-only; the SDK and
published packages are MIT licensed. [LICENSE](./LICENSE) is the authoritative
map and currently identifies some first-party paths whose license is not yet
assigned. Confirm the intended license with a maintainer before contributing to
an unmapped path. By submitting a contribution to a licensed path, you agree
