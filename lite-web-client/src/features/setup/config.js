...
export function serializeManagedConfig(form) {
  ...
  const smtp = orderedObject(
    SMTP_KEYS,
    {
      ...
      host: trimString(config.smtp?.host),
      port: parsePositiveInt(config.smtp?.port, DEFAULT_CONFIG.smtp.port),
      username: trimString(config.smtp?.username),
      password: trimString(config.smtp?.password),
