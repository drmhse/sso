...
import type { OAuthButtonProps, SupportedOAuthProvider } from '../types';
 ...
 */
function getProviderIcon(provider: SupportedOAuthProvider) {
  switch (provider) {
    case 'github':
      return (
    ...
    >
      {children || (
        <>
          ...
          <span>Continue with {PROVIDER_NAMES[provider]}</span>
        </>
      )}
    </button>
  );
