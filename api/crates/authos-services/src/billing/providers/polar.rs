...
impl BillingProvider for PolarProvider {
    ...
    async fn create_customer(&self, request: CreateCustomerRequest) -> Result<String> {
        // Polar has no customer-creation endpoint: it creates the record on the
        // first checkout, keyed by the external_customer_id we return here.
...
impl PolarProvider {
    ...
    fn parse_subscription_event(
        ...
        let current_period_end = data
            ...
            .map_or_else(Utc::now, |dt| dt.with_timezone(&chrono::Utc));
