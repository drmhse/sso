...
impl MfaAuditService {
    ...
    /// Log an MFA audit event. `org_id` is `None` for platform-level events.
    ...
    pub async fn log_mfa_event(
        ...
        crate::audit::actor::enqueue_mfa_with_connection(&self.db, audit_log).await?;
    ...
    pub async fn log_org_event(
        ...
        let audit_log = OrgAuditLogActiveModel {
            ...
            actor_user_id: Set(actor_user_id
                .map_or_else(|| "system".to_string(), std::string::ToString::to_string)),
            action: Set(action.clone()),
            target_type: Set(
                target_type.map_or_else(|| "unknown".to_string(), std::string::ToString::to_string)
            ),
        ...
        crate::audit::actor::enqueue_org_with_connection(&self.db, audit_log).await?;
