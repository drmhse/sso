...
impl TierService {
    ...
    pub async fn check_feature_access(
        ...
        // Org overrides take precedence over the tier defaults.
        if let Some(overrides) = &org.feature_overrides {
            // Malformed overrides fall through to the tier defaults. Overrides
            // only ever widen access, so ignoring one cannot grant more.
