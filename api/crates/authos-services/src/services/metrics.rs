...
impl MfaMetricsService {
    ...
    pub async fn record_feature_usage(
        ...
        let active_model = mfa_feature_usage::ActiveModel {
            ...
            timestamp: Set(now),
            ip_address: Set(ip_address.map(std::string::ToString::to_string)),
            user_agent: Set(user_agent.map(std::string::ToString::to_string)),
            success: Set(success),
            details: Set(details.map(std::string::ToString::to_string)),
        };
    ...
    pub async fn record_failure(
        ...
        if let Some(pattern) = existing_pattern {
            ...
            if details.is_some() {
                active_pattern.details = Set(details.map(std::string::ToString::to_string));
            }
            ...
            let new_pattern = mfa_failure_patterns::ActiveModel {
                id: Set(pattern_id),
                org_id: Set(org_id.map(std::string::ToString::to_string)),
                user_id: Set(user_id.map(std::string::ToString::to_string)),
                ip_address: Set(ip_address.map(std::string::ToString::to_string)),
    ...
    pub async fn get_mfa_metrics(
        ...
        Ok(metrics.into_iter().map(Self::to_summary).collect())
    }

    fn to_summary(m: mfa_daily_metrics::Model) -> MfaMetricsSummary {
        MfaMetricsSummary {
            org_id: m.org_id,
            date: m.date,
    ...
    /// Same as `get_mfa_metrics` but over an explicit inclusive date range.
    pub async fn get_mfa_metrics_in_range(
        &self,
        org_id: Option<&str>,
        ...
        let mut query = mfa_daily_metrics::Entity::find()
            .filter(mfa_daily_metrics::Column::Date.gte(start_date.format("%Y-%m-%d").to_string()))
            .filter(mfa_daily_metrics::Column::Date.lte(end_date.format("%Y-%m-%d").to_string()));

        // A null org_id is the platform-wide row, not "any organization".
        if let Some(oid) = org_id {
            query = query.filter(mfa_daily_metrics::Column::OrgId.eq(oid));
        } else {
            query = query.filter(mfa_daily_metrics::Column::OrgId.is_null());
...
mod tests {
    ...
    async fn daily_metrics_counts_total_and_mfa_users_in_one_aggregate_shape() {
        ...
        let user_with_mfa = crate::test_support::insert_user(&db, Some("mfa@example.com")).await;
        let user_without_enabled_mfa =
            crate::test_support::insert_user(&db, Some("disabled@example.com")).await;
        let outside_user_with_mfa =
            crate::test_support::insert_user(&db, Some("outside@example.com")).await;
        let org_id = crate::test_support::insert_org(&db, &user_with_mfa).await;

        crate::test_support::insert_membership(&db, &org_id, &user_with_mfa).await;
        crate::test_support::insert_membership(&db, &org_id, &user_without_enabled_mfa).await;
