...
impl RiskEngine {
    pub fn new() -> Result<Self> {
        ...
        let signing_key = parse_device_trust_secret(
    ...
    pub async fn evaluate(&self, db: DB<'_>, ctx: RiskContext<'_>) -> Result<RiskAssessment> {
        ...
        let new_device_score = rules.as_ref().map_or(20, |r| r.new_device_score);
        let impossible_travel_score = rules.as_ref().map_or(50, |r| r.impossible_travel_score);
        let velocity_threshold = rules.as_ref().map_or(10, |r| r.velocity_threshold);
        let velocity_score = rules.as_ref().map_or(30, |r| r.velocity_score);
