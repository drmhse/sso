...
impl OAuthStateStore {
    ...
    pub async fn create(
        ...
        let new_state = oauth_states::ActiveModel {
            ...
            pkce_verifier: Set(pkce_verifier.map(std::string::ToString::to_string)),
            service_id: Set(service_id.map(std::string::ToString::to_string)),
            redirect_uri: Set(redirect_uri.map(std::string::ToString::to_string)),
            org_slug: Set(org_slug.map(std::string::ToString::to_string)),
            service_slug: Set(service_slug.map(std::string::ToString::to_string)),
            is_admin_flow: Set(is_admin_flow),
            user_id_for_linking: Set(user_id_for_linking.map(std::string::ToString::to_string)),
            device_user_code: Set(device_user_code.map(std::string::ToString::to_string)),
            saml_state_id: Set(saml_state_id.map(std::string::ToString::to_string)),
            upstream_connection_id: Set(
                upstream_connection_id.map(std::string::ToString::to_string)
            ),
            requested_scopes: Set(requested_scopes_json),
            client_state: Set(client_state.map(std::string::ToString::to_string)),
            provider_token_request_state: Set(
                provider_token_request_state.map(std::string::ToString::to_string)
            ),
