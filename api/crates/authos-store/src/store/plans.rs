...
impl PlanStore {
    ...
    pub async fn create(
        ...
        let new_plan = plans::ActiveModel {
            ...
            stripe_price_id: Set(stripe_price_id.map(std::string::ToString::to_string)),
    ...
    pub async fn update(
        ...
        if let Some(d) = description {
            active_plan.description = Set(d.map(std::string::ToString::to_string));
        }
        ...
        if let Some(sp) = stripe_price_id {
            active_plan.stripe_price_id = Set(sp.map(std::string::ToString::to_string));
        }
