...
impl OrganizationStore {
    ...
    pub async fn create(
        ...
        let new_org = organizations::ActiveModel {
            ...
            tier_id: Set(tier_id.map(std::string::ToString::to_string)),
            ..Default::default()
        };
    ...
    pub async fn reject(
        ...
        org_active.rejection_reason = Set(reason.map(std::string::ToString::to_string));
        org_active.updated_at = Set(now);
    ...
    pub async fn update_smtp_config(
        ...
        org_active.smtp_from_name = Set(from_name.map(std::string::ToString::to_string));
        org_active.smtp_encryption_key_id =
            Set(encryption_key_id.map(std::string::ToString::to_string));
        org_active.updated_at = Set(chrono::Utc::now().naive_utc());
    ...
    pub async fn get_oauth_client_for_org(
        ...
        // Load configuration
        let config = Config::from_env().map_err(AppError::InternalServerError)?;
    ...
    pub async fn update_feature_overrides(
        ...
        let mut org_active: organizations::ActiveModel = org.into();
        org_active.feature_overrides = Set(overrides_json.map(std::string::ToString::to_string));
        org_active.updated_at = Set(chrono::Utc::now().naive_utc());
...
mod store_tests {
    ...
    async fn status_transitions_and_the_pending_gate() {
        ...
        match ensure_organization_active(&db, &org.id).await {
...
/// Load an organization and reject it unless it is active. Pending and suspended
/// tenants must not reach handler logic.
