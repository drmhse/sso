...
impl UpstreamProviderStore {
    ...
    pub async fn create(
        ...
        let provider = upstream_providers::ActiveModel {
            ...
            authorization_url: Set(authorization_url.map(std::string::ToString::to_string)),
            token_url: Set(token_url.map(std::string::ToString::to_string)),
            userinfo_url: Set(userinfo_url.map(std::string::ToString::to_string)),
            discovery_url: Set(discovery_url.map(std::string::ToString::to_string)),
            scopes: Set(scopes.map(std::string::ToString::to_string)),
            issuer: Set(issuer.map(std::string::ToString::to_string)),
