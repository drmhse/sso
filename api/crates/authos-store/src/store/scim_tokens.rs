...
mod tests {
    ...
    async fn setup_db() -> (DatabaseConnection, String, String) {
        ...
        let user_id = crate::test_support::insert_user(&db, None).await;
        let org_id = crate::test_support::insert_org(&db, &user_id).await;
        (db, org_id, user_id)
    }
    ...
    async fn scim_token_lifecycle_is_org_scoped_and_preserves_other_tenant() {
        ...
        let org_b = crate::test_support::insert_org(&db, &user_id).await;
        let (_, prefix, hash) = ScimTokenStore::generate();
        let token = ScimTokenStore::create(
            DB::Conn(&db),
            &org_b,
