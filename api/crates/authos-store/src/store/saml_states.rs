...
impl SamlStateStore {
    ...
    pub async fn create(
        ...
        let new_state = saml_states::ActiveModel {
            ...
            relay_state: Set(relay_state.map(std::string::ToString::to_string)),
            acs_url: Set(acs_url.to_string()),
            request_id: Set(request_id.map(std::string::ToString::to_string)),
            issuer: Set(issuer.map(std::string::ToString::to_string)),
            binding: Set(binding.map(std::string::ToString::to_string)),
            user_id: Set(None),
            created_at: Set(now),
