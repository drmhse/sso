...
impl ScimOperator {
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            "eq" => Some(ScimOperator::Equals),
            "ne" => Some(ScimOperator::NotEquals),
            "co" => Some(ScimOperator::Contains),
            "sw" => Some(ScimOperator::StartsWith),
            "ew" => Some(ScimOperator::EndsWith),
            "pr" => Some(ScimOperator::Present),
            "gt" => Some(ScimOperator::GreaterThan),
            "ge" => Some(ScimOperator::GreaterThanOrEqual),
            "lt" => Some(ScimOperator::LessThan),
            "le" => Some(ScimOperator::LessThanOrEqual),
            _ => None,
        }
    }
...
impl ScimFilterParser {
    ...
    pub fn parse(filter: &str) -> Result<Vec<ScimFilterExpression>> {
        ...
        // Handle logical operators (basic AND support for now)
    ...
    fn parse_single_expression(expr: &str) -> Result<ScimFilterExpression> {
        ...
        for op_str in &operators {
            if expr.contains(op_str) {
                ...
                if parts.len() == 2 {
                    let attribute_path = parts[0].trim().to_string();
                    let operator = ScimOperator::from_str(op_str.trim()).ok_or_else(|| {
