...
impl AuditHandle {
    ...
    #[cfg(test)]
    pub(crate) fn without_worker(db: DatabaseConnection) -> Self {
...
mod tests {
    ...
    async fn user(database: &DatabaseConnection, email: &str) -> crate::entities::users::Model {
        crate::store::users::UserStore::create(crate::store::DB::Conn(database), email, None, true)
            .await
            .unwrap()
    }
