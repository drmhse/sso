...
mod tests {
    ...
    async fn refresh_preserves_session_resource_audience() {
        ...
        let refresh = crate::crypto::refresh_tokens::generate();
        let now = Utc::now();
        SessionStore::create(
            DB::Conn(&state.db),
            &user.id,
        ...
        assert_eq!(
            ...
            Some(crate::crypto::refresh_tokens::hash(&response.refresh_token).as_str())
        );
        ...
        let expired_access = state
            ...
            .unwrap();
        let expired_refresh = crate::crypto::refresh_tokens::generate();
        ...
        let logout_access = state
            ...
            .create_token(&user.id, &user.email, false, Some(&org.slug), None)
            .unwrap();
        let logout_refresh = crate::crypto::refresh_tokens::generate();
