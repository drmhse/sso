...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let user_rows = db.query_all(Statement::from_string(
            backend,
            "SELECT id, email FROM users WHERE org_id IS NULL AND is_platform_owner = FALSE".to_owned()
        )).await?;
        ...
        let identity_rows = db.query_all(Statement::from_string(
            backend,
            "SELECT i.user_id, s.org_id 
             ...
             FROM identities
             WHERE issuing_org_id IS NOT NULL".to_owned()
        )).await?;
                ...
                if orgs.len() == 1 {
                    ...
                    for &other_org_id in orgs.iter().skip(1) {
                        ...
                        db.execute(Statement::from_sql_and_values(
                            backend,
                            &update_sql,
                            vec![new_user_id.into(), user.id.clone().into(), other_org_id.into(), other_org_id.into()]
                        )).await?;
                    }
                }
            }
