...
impl Migration {
    ...
    async fn up_sqlite<'a>(
        ...
        if !users_exists && users_new_exists {
            ...
            if has_org_id {
                ...
                // First, save the data
                db.execute_unprepared("ALTER TABLE users_new RENAME TO users_old_backup").await?;
            ...
            db.execute_unprepared("DROP TABLE IF EXISTS users_new").await?;
    ...
    async fn table_exists<'a>(&self, db: &SchemaManagerConnection<'a>, table_name: &str) -> Result<bool, DbErr> {
    ...
    async fn column_exists<'a>(&self, db: &SchemaManagerConnection<'a>, table_name: &str, column_name: &str) -> Result<bool, DbErr> {
    ...
    async fn ensure_indexes<'a>(&self, manager: &SchemaManager<'a>, db: &SchemaManagerConnection<'a>) -> Result<(), DbErr> {
    ...
    async fn down_sqlite<'a>(
        ...
        let _ = manager.drop_index(
            Index::drop().name("idx_users_org_id").table(Users::Table).to_owned()
        ).await;
        let _ = db.execute_unprepared("DROP INDEX IF EXISTS idx_users_email_org").await;
        let _ = db.execute_unprepared("DROP INDEX IF EXISTS idx_users_email_platform").await;
    ...
    async fn up_postgres<'a>(
        ...
        db.execute_unprepared("ALTER TABLE users DROP CONSTRAINT IF EXISTS users_email_key").await?;
    ...
    async fn down_postgres<'a>(
        ...
        db.execute_unprepared("DROP INDEX IF EXISTS idx_users_email_org").await?;
        db.execute_unprepared("DROP INDEX IF EXISTS idx_users_email_platform").await?;
        ...
        db.execute_unprepared("ALTER TABLE users ADD CONSTRAINT users_email_key UNIQUE (email)").await?;
    ...
    async fn down_mysql<'a>(
        ...
        db.execute_unprepared("DROP INDEX idx_users_email_org ON users").await?;
        ...
        let _ = db.execute_unprepared("CREATE UNIQUE INDEX email ON users (email)").await;
