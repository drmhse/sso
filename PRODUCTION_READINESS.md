...
- One Product maintainer and one Security or Release reviewer sign off on the public claim audit.
...
- Exercise OIDC discovery, authorization-code flow with PKCE, token validation, refresh, logout/revocation, and a basic organization-isolation scenario in smoke tests.
...
- Test OAuth/OIDC abuse cases including redirect URI validation, PKCE enforcement, state/nonce handling, authorization-code reuse, refresh-token replay and rotation, revocation, audience/issuer confusion, algorithm confusion, JWKS caching, signing-key rollover, and logout behavior.
- Run an applicable OpenID/OAuth conformance suite against a release candidate. Publish the profile, configuration, results, failures, and justified exclusions; do not use a generic “OIDC compliant” claim beyond tested profiles.
- Build SAML interoperability fixtures for at least three independent identity-provider implementations and test signature validation, assertion conditions, replay, audience, destination, clock skew, metadata rollover, and certificate rotation.
...
- SAML tests pass against at least three independent IdPs; SCIM tests pass against at least two independent clients.
